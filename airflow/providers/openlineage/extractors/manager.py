...
_extractors: list[BaseExtractor] = list(
    filter(
        ...
        [
            ...
            try_import_from_string("airflow.providers.openlineage.extractors.mysql_extractor.MySqlExtractor"),
            try_import_from_string(
                "airflow.providers.openlineage.extractors.bigquery_extractor.BigQueryExtractor"
            ),
            try_import_from_string(
                "airflow.providers.openlineage.extractors.great_expectations_extractor.GreatExpectationsExtractor"
            ),
            try_import_from_string(
            ...
            try_import_from_string("airflow.providers.openlineage.extractors.bash_extractor.BashExtractor"),
            try_import_from_string(
            ...
            try_import_from_string("airflow.providers.openlineage.extractors.trino_extractor.TrinoExtractor"),
            try_import_from_string(
                "airflow.providers.openlineage.extractors.athena_extractor.AthenaExtractor"
            ),
            try_import_from_string("airflow.providers.openlineage.extractors.sftp_extractor.SFTPExtractor"),
            try_import_from_string("airflow.providers.openlineage.extractors.ftp_extractor.FTPExtractor"),
            try_import_from_string(
...
_extractors += get_check_extractors(
    try_import_from_string("airflow.providers.openlineage.extractors.sql_extractor.SqlExtractor")
)
    ...
    def instantiate_abstract_extractors(self, task) -> None:
        ...
        if task.__class__.__name__ in (
            ...
            for extractor in self.extractors.values():
                ...
                raise ValueError(
                    "Extractor for the given task's conn_type (%s) does not exist.", task_conn_type
                )
        ...
        if task.__class__.__name__ == "SQLExecuteQueryOperator":
            ...
            extract_type: list[type[BaseExtractor]] = list(
                filter(lambda x: x.__name__ == extractor_name, self.extractors.values())
            )
            if extract_type:
                self.extractors["SQLExecuteQueryOperator"] = get_sql_execute_query_extractor(  # type: ignore
                    extract_type[0]
                )
        return
